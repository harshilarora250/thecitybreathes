...
6. **Sculpture Mapping**: Data values drive specific sculpture parameters:
   - **Traffic** → Breathing speed, vertical deformation, rotation acceleration
...
- **City Selector**: Dropdown menu with five major world cities.
...
A living 3D sculpture that breathes, sings, and transforms with real-time city data.

## Overview

The City Breathes turns traffic, air quality, weather, and social activity into an interactive digital sculpture that could not exist without technology.

## Features

- Real-time city data integration from public APIs
- 3D breathing sculpture with morphing geometry
- Data-driven color, motion, and particle effects
- Interactive breathing style controls
- Generative audio using Web Audio API
- City selector and data visualization
- Fallback demo mode for reliable presentations
...
3. Run `npm start` to start the static server
4. Navigate to `http://localhost:5173` in your browser

No API keys are required for basic operation. The application uses free public APIs with no authentication.
